...
class ImageHandler extends HTMLElement {
  ...
  setupEventListeners() {
    ...
    imageEditor.addEventListener('image-saved', (e) => {
      ...
      const oldImage = this.images.find((img) => img.id === imageId);
      if (oldImage && oldImage.preview && oldImage.preview.startsWith('blob:')) {
  ...
  removeImage(imageId) {
    ...
    if (removedImage && removedImage.preview && removedImage.preview.startsWith('blob:')) {
