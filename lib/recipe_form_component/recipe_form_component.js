class RecipeFormComponent extends HTMLElement {
  ...
  async setRecipeData(recipeId) {
    try {
      ...
      if (doc.exists) {
        ...
        // Add ingredient entries based on fetched data
        data.ingredients.forEach((ingredient, index) => {
          if (index > 0) {
            this.addIngredientLine({ target: ingredientsContainer.querySelector('.recipe-form__button--add-ingredient') });
          }
  ...
  async fetchImageFromStorage(recipeId, category) {
    ...
    const imagePath = `img/recipes/compressed/${category}/${fileName}`;
