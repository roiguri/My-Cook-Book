class RecipeFormComponent extends HTMLElement {
  ...
  setupEventListeners() {
    ...
    imageInput.addEventListener('change', () => this.showImagePreview());
  ...
  collectFormData() {
    this.recipeData = {
      ...
      cookingTime: parseInt(this.shadowRoot.getElementById('prep-time').value) + parseInt(this.shadowRoot.getElementById('wait-time').value),
    ...
    try {
      ...
      if (doc.exists) {
        ...
        data.ingredients.forEach((ingredient, index) => {
          if (index > 0) { 
            this.addIngredientLine({ target: ingredientsContainer.querySelector('button') });
          }
          const entry = ingredientsContainer.querySelectorAll('.recipe-form__ingredient-entry')[index];
        ...
        stagesContainer.querySelectorAll('.recipe-form__steps').forEach((stage, index) => {
          if (index > 0) { // Keep the first stage
            stage.remove();
          }
        });

        data.stages.forEach((stage, stageIndex) => {
          if (stageIndex > 0) { // Add new stages for additional stages
            this.addStage({ target: stagesContainer.querySelector('button') });
          }
          const currentStage = stagesContainer.querySelectorAll('.recipe-form__steps')[stageIndex];
          currentStage.querySelectorAll('.recipe-form__step').forEach((step, stepIndex) => {
            if (stepIndex > 0) { // Add new steps for additional steps within the stage
              this.addStepLine({ target: currentStage.querySelectorAll('button')[stepIndex - 1] });
            }
            currentStage.querySelectorAll('.recipe-form__step')[stepIndex].querySelector('input[type="text"]').value = stage.instructions[stepIndex];
          });
        });  

        // Fetch image from Firebase Storage
        const imageUrl = await this.fetchImageFromStorage(recipeId, data.category);
        if (imageUrl) {
          const previewImage = this.shadowRoot.getElementById('image-preview');
  ...
  async fetchImageFromStorage(recipeId, category) {
    const storage = firebase.storage();
    const fileName = `${recipeId}.jpg`; // Assuming the image extension is always JPG
    const imagePath = `img/recipes/full/${category}/${fileName}`;
