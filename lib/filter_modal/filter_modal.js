 ...
 * with multiple criteria. Integrates with Firebase/Firestore for data management.
 * 
 * @example
 * // HTML
 ...
 * });
 * 
 * @fires filter-applied - When filters are applied, contains filtered recipes and current filter state
 * @fires filter-reset - When filters are reset, contains all applicable recipes
 * 
 * @attr {string} category - Optional category to filter by
 * @attr {boolean} cooking-time-filter - Enable/disable cooking time filter
 * @attr {boolean} difficulty-filter - Enable/disable difficulty filter
 * @attr {boolean} ingredient-filter - Enable/disable main ingredient filter
 * @attr {boolean} tags-filter - Enable/disable tags filter
 */
...
class RecipeFilterComponent extends HTMLElement {
  ...
  render() {
    this.shadowRoot.innerHTML = `
      ...
      <custom-modal height="600px" width="800px">
  ...
  styles() {
    return `
      ...
      /* Update existing filter-section style to accommodate new structure */
      .filter-section {
        background: #f5f5f5;
        padding: 15px;
        border-radius: 8px;
  ...
  async loadInitialData() {
    ...
    try {
      ...
      // Extract unique main ingredients and tags
      this.state.availableFilters.mainIngredients = [...new Set(
        recipes
          .map(r => r.mainIngredient)
          .filter(ingredient => ingredient && ingredient.trim()) // Remove empty/null values
