...
class ImageApprovalComponent extends HTMLElement {
  ...
  async handleApprove() {
    ...
    try {
      // Move the image from pendingImages to the appropriate recipe folder
      ...
      const pendingImageRef = storage.ref(`pendingImages/${this.imageData.recipeId}.jpg`);
      const path = `img/recipes/compressed/${recipeData.category}/${recipeData.image}`;
      const newImageRef1 = storage.ref(`img/recipes/compressed/${recipeData.category}/${recipeData.image}`);
      const newImageRef2 = storage.ref(`img/recipes/full/${recipeData.category}/${recipeData.image}`)
      
      const imageBlob = await pendingImageRef.getDownloadURL().then(url => fetch(url, { mode: 'no-cors' }).then(res => res.blob()));
      await newImageRef1.put(imageBlob);
      await newImageRef2.put(imageBlob);
