class ElementScroller extends HTMLElement {
  ...
  */
  ...
  handleTouchEnd() {
      ...
      if (Math.abs(velocity) > 0.5) { // Adjust this threshold as needed
          // Calculate how many items to scroll based on velocity
          const itemsToScroll = Math.min(
              Math.ceil(Math.abs(velocity) * 2), // Adjust multiplier for sensitivity
              this.getVisibleItems()
          );
          
          if (velocity < 0) {
              this.scrollRight(itemsToScroll);
          } else {
              this.scrollLeft(itemsToScroll);
          }
      } else {
      ...
      this.touchState.prevTranslate = this.touchState.currentTranslate;
  }
