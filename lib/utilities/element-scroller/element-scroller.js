class ElementScroller extends HTMLElement {
  ...
  */
  ...
  /**
   * Updates scroll indicator width and position
   */
  updateScrollIndicator() {
    if (!this.hasAttribute('show-scrollbar')) return;
    ...
    const visibleRatio = containerWidth / totalWidth;
    const indicatorWidth = Math.max(visibleRatio * 100, 10); 

    // Calculate scroll percentage based on current index relative to max scroll
    const maxScroll = this.getMaxScroll();
    let scrollPercentage = maxScroll > 0 ? 
        (this.currentIndex / maxScroll) * (100 - indicatorWidth) : 0;

    // If we're at max scroll, ensure we reach the end
    if (this.currentIndex === maxScroll) {
        scrollPercentage = 100 - indicatorWidth;
    }
