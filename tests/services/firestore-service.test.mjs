...
describe('FirestoreService', () => {
  describe('getDocument', () => {
    ...
    it('throws on error', async () => {
      ...
      firebaseServiceMocks.userDocMock.exists = true;
      firebaseServiceMocks.userDocMock.data = () => { throw new Error('fail'); };
      const errorSpy = jest.spyOn(console, 'error').mockImplementation(() => {});
      await expect(FirestoreService.getDocument('users', 'doc1')).rejects.toThrow('Failed to fetch document');
  ...
  describe('updateDocument', () => {
    it('calls updateDoc with correct args', async () => {
      ...
      await expect(FirestoreService.updateDocument('users', 'id', { foo: 1 })).resolves.toBeUndefined();
  ...
  describe('batchWrite', () => {
    it('commits batch with correct operations', async () => {
      ...
      await expect(FirestoreService.batchWrite([
